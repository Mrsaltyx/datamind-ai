...
from datamind.analysis.dtypes import categorical_columns
...
def suggest_models(df: pd.DataFrame, target_col: str, task_type: str) -> list:
    ...
    has_high_cardinality = any(df[c].nunique() > 30 for c in cat_cols if c != target_col)
...
def suggest_evaluation_strategy(
    ...
    # 5 folds est un bon compromis biais/variance pour les tailles usuelles ;
    # les cas extremes (tres petit / tres grand) sont geres via method ci-dessous.
    n_folds = 5
